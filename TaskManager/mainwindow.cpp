    ...
    , ui(new Ui::MainWindow)
{
    ...
    QPlainTextEdit *book = ui->inBook;
    book->verticalScrollBar()->setStyleSheet("QScrollBar:vertical { background: #CCCCCC; width: 1px; }"
...
{
    QPlainTextEdit *book = ui->inBook;
    QTextCursor cursor = book->textCursor();

    cursor.movePosition(QTextCursor::StartOfLine);
    cursor.movePosition(QTextCursor::EndOfLine, QTextCursor::KeepAnchor);

    QTextCharFormat format;
    format.setFontPointSize(35);
    cursor.mergeCharFormat(format);

    book->setTextCursor(cursor);
}
